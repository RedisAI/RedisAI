...
def test_dag_modelrun_financialNet(env):
    ...
    for reference_tensor in creditcard_referencedata:
        ret = con.execute_command('AI.DAGRUN', 'PERSIST', '1', 'referenceTensor:{0}'.format(tensor_number), '|>',  'AI.TENSORSET', 'referenceTensor:{0}'.format(tensor_number),
                                  'FLOAT', 1, 256,
    ...
    for transaction_tensor in creditcard_transactions:
        # 'PERSIST', '1', 'classificationTensor:{0}'.format(tensor_number), 
        ret = con.execute_command(
            'AI.DAGRUN', 'LOAD', '1', 'referenceTensor:{}'.format(tensor_number), '|>',
            'AI.TENSORSET', 'transactionTensor:{0}'.format(tensor_number), 'FLOAT', 1, 30,'BLOB', transaction_tensor.tobytes(), '|>',
