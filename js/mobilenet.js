...
async function run(filenames) {
  ...
  redis.call('TF.GRAPH', 'mobilenet', buffer);
  ...
  for (i in filenames) {
    ...
    redis.call('TF.TENSOR', 'input_' + i, 'FLOAT', 4, 1, image_width, image_height, 3, 'BLOB', buffer);

    console.log("Running graph");
    redis.call('TF.RUN', 'mobilenet', 1, 'input_' + i, input_var, 'output_' + i, output_var);

    console.log("Getting output tensor");
    let out_data = await redis.callBuffer('TF.DATA', 'output_' + i);
