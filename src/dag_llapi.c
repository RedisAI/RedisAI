...
int RAI_DagRunAsync(RedisAI_RunInfo *run_info, RAI_OnFinishCB DAGAsyncFinish, void *private_data) {
...
int RAI_ModelRunAsync(RAI_ModelRunCtx *mctxs, RAI_OnFinishCB ModelAsyncFinish, void *private_data) {
    ...
    RAI_Error error = {0};
    RedisAI_RunInfo *rinfo = Dag_CreateFromSingleModelRunOp(mctxs, &error, NULL, NULL, NULL, 0);
    if (rinfo == NULL)
