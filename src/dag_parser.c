...
int RAI_parseDAGLoadArgs(RedisModuleCtx *ctx, RedisModuleString **argv, int argc,
  AI_dict **loadedContextDict, AI_dict **localContextDict, const char *chaining_operator) {
	if (argc < 3) {
  ...
  RedisAI_RunInfo **rinfo_ptr) {
	...
	for (; arg_pos < argc; arg_pos++) {
		...
		if (!strcasecmp(arg_string, "LOAD") && !load_complete) {
			...
			const int parse_result = RAI_parseDAGLoadArgs(ctx, &argv[arg_pos],
			  argc - arg_pos, &(rinfo->dagTensorsLoadedContext),
			...
			const int parse_result = RAI_parseDAGPersistArgs(ctx, &argv[arg_pos],
			  argc - arg_pos, &(rinfo->dagTensorsPersistedContext), "|>");
			if (parse_result > 0) {
				arg_pos += parse_result - 1;
