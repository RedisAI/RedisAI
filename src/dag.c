...
static void Dag_LoadInputsToModelRunCtx(RedisAI_RunInfo *rinfo, RAI_DagOp *currentOp) {
        ...
        for (int i = 0; i < ninputs1; i++) {
            ...
            if (rinfo1->single_op_dag == 1) {
                ...
                RAI_getTensorFromLocalContext(NULL, rinfo1->dagTensorsContext,
                                              RedisModule_StringPtrLen(op1->inkeys[i], NULL),
                                              &input1, op1->err);
            }
            RAI_Tensor *input2;
            if (rinfo2->single_op_dag == 1) {
                ...
                RAI_getTensorFromLocalContext(NULL, rinfo2->dagTensorsContext,
                                              RedisModule_StringPtrLen(op2->inkeys[i], NULL),
                                              &input2, op2->err);
            }
    ...
    void RedisAI_DagOpBatchInfo(RedisAI_RunInfo * rinfo, RAI_DagOp * op, size_t * batchsize,
                                size_t * minbatchsize, size_t * minbatchtimeout,
                                size_t * inbatchsize) {
        *batchsize = 0;
        *minbatchsize = 0;
        *minbatchtimeout = 0;
    ...
    void RedisAI_DagOpBatchingMatch(RedisAI_RunInfo * rinfo1, RAI_DagOp * op1,
                                    RedisAI_RunInfo * rinfo2, RAI_DagOp * op2, int *batched,
                                    size_t *inbatchsize) {
        *batched = 0;
        *inbatchsize = 0;
    ...
    static int StoreTensorInKeySpace(RedisModuleCtx * ctx, RAI_Tensor * tensor,
        ...
        if (status == REDISMODULE_ERR) {
            ...
            if (RedisModule_ModuleTypeSetValue(
                    key, RedisAI_TensorType, RAI_TensorGetShallowCopy(tensor)) != REDISMODULE_OK) {
                RedisModule_ReplyWithError(ctx, "ERR could not save tensor");
                goto clean_up;
            }
        }
    ...
    static void PersistTensors(RedisModuleCtx * ctx, RedisAI_RunInfo * rinfo) {
        ...
        while (persist_entry) {
            ...
            if (tensor_entry) {
                ...
                bool mangled = true;
                if (StoreTensorInKeySpace(ctx, tensor, persist_key_name, mangled) ==
                    REDISMODULE_ERR)
                ...
                for (size_t opN = 0; opN < array_len(rinfo->dagOps); opN++) {
                    RedisModule_Log(ctx, "warning", "DAG's op n#  %zu - cmdType %d ( argc %d )",
    ...
    int RedisAI_DagRun_Reply(RedisModuleCtx * ctx, RedisModuleString * *argv, int argc) {
            ...
            case REDISAI_DAG_CMD_TENSORGET: {
                ...
                if (currentOp->result == REDISMODULE_ERR) {
                    ...
                    if (array_len(currentOp->outTensors) > 0) {
                        ...
                        RedisModule_ReplyWithError(ctx,
                                                   "ERR error getting tensor from local context");
                    }
                }
                break;
            }
        ...
        if (rinfo->single_op_dag == 0 ||
            rinfo->dagOps[0]->commandType == REDISAI_DAG_CMD_SCRIPTRUN) {
    ...
    int RedisAI_ProcessDagRunCommand(RedisModuleCtx * ctx, RedisModuleString * *argv, int argc,
        ...
        bool blocking_not_allowed =
            (flags & (REDISMODULE_CTX_FLAGS_MULTI | REDISMODULE_CTX_FLAGS_LUA));
        ...
        rinfo->client =
            RedisModule_BlockClient(ctx, RedisAI_DagRun_Reply, NULL, RunInfo_FreeData, 0);
